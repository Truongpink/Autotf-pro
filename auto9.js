    ...
    return new Promise(resolve => {
        $httpClient.get({url: testurl + ID, headers: header}, (error, response, data) => {
            ...
            if (jsonData.data.status === 'FULL') {
                ...
                return; // Kết thúc hàm nếu chương trình beta đã đầy
            }

            // Thông báo khi có chỗ trống trong beta
            $notification.post(`${ID} Có chỗ trống trong beta`, '', `${ID} Có chỗ trống trong beta`);
